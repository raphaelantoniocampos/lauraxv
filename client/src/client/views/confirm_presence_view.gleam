...
pub fn confirm_presence_view(model: model.Model) -> Element(Msg) {
  main([class("w-full max-w-2xl p-8 mt-20 bg-white rounded-lg shadow-lg")], [
    case model.is_confirmed {
      Some(True) -> confirmed_user_view()
      _ -> unconfirmed_user_view(model)
    },
  ])
}
...
fn unconfirmed_user_view(model: model.Model) -> Element(Msg) {
  div([class("p-2 mt-6 mx-4")], [
    ...
    form(
      ...
      [
        ...
        div([], [
          label(
            [class("block text-sm font-medium text-gray-700"), for("comments")],
