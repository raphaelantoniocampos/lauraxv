...
-type message(KRH) :: {internal, internal_message()} | {user, KRH}.

-type loop_message(KRI) :: {packet, bitstring()} | {custom, KRI}.

-type loop_state(KRJ, KRK) :: {loop_state,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(KRJ)),
        glisten@transport:transport(),
        KRK}.

-type connection(KRL) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(KRL))}.

-type handler(KRM, KRN) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(KRM), KRN, connection(KRM)) -> gleam@otp@actor:next(loop_message(KRM), KRN)),
        fun((connection(KRM)) -> {KRN,
            gleam@option:option(gleam@erlang@process:selector(KRM))}),
        gleam@option:option(fun((KRN) -> nil)),
        glisten@transport:transport()}.

-file("/home/raphaelac/repositories/lauraxv/server/build/packages/glisten/src/glisten/internal/handler.gleam", 71).
-spec start(handler(KRX, any())) -> {ok,
        gleam@erlang@process:subject(message(KRX))} |
