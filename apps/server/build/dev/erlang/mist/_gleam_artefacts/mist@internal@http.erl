...
-export([from_header/1, read_data/4, add_default_headers/3, handle_continue/1, parse_headers/4, crypto_hash/2, base64_encode/1, parse_chunk/1, parse_request/2, read_body/1, upgrade_socket/2, upgrade/4]).
-export_type([response_data/0, connection/0, packet_type/0, http_uri/0, http_packet/0, decoded_packet/0, decode_error/0, chunk/0, parsed_request/0, body/0, sha_hash/0]).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 80).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 115).
-spec read_data(
    glisten@socket:socket(),
    glisten@transport:transport(),
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 485).
-spec add_default_headers(
    gleam@http@response:response(gleam@bytes_builder:bytes_builder()),
    boolean(),
    boolean()
) -> gleam@http@response:response(gleam@bytes_builder:bytes_builder()).
add_default_headers(Resp, Keep_alive, Is_head_response) ->
    ...
    Headers@1 = case {Include_content_length, Is_head_response} of
        {true, true} ->
    ...
    Existing = gleam@list:key_pop(Headers@1, <<"connection"/utf8>>),
    Keep_alive_header = {<<"connection"/utf8>>, <<"keep-alive"/utf8>>},
    Headers@3 = case {Keep_alive, Existing} of
        {_, {ok, {Connection, Headers@2}}} ->
            [{<<"connection"/utf8>>, Connection} | Headers@2];

        {false, _} ->
            Headers@1;

        {true, _} ->
            [Keep_alive_header | Headers@1]
    end,
    Headers@4 = (case gleam@http@response:get_header(Resp, <<"date"/utf8>>) of
        {error, _} ->
            [{<<"date"/utf8>>, mist@internal@clock:get_date()} | Headers@3];

        _ ->
            Headers@3
    end),
    erlang:setelement(3, Resp, Headers@4).

-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 529).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 537).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 86).
-spec parse_headers(
    bitstring(),
    glisten@socket:socket(),
    glisten@transport:transport(),
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 558).
-spec crypto_hash(sha_hash(), binary()) -> binary().
crypto_hash(Hash, Data) ->
    crypto:hash(Hash, Data).

-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 561).
-spec base64_encode(binary()) -> binary().
base64_encode(Data) ->
    base64:encode(Data).

-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 148).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 243).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 178).
-spec read_chunk(
    glisten@socket:socket(),
    glisten@transport:transport(),
    mist@internal@buffer:buffer(),
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 335).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 416).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 423).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/http.gleam", 463).
-spec upgrade(
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary()),
        ...
        fun(Resp) ->
            gleam@result:then(
                begin
                    ...
                    _pipe@4 = glisten@transport:send(Transport, Socket, _pipe@3),
