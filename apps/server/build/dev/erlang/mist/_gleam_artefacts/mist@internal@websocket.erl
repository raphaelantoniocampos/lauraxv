...
-type valid_message(NYM) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, NYM}.

-type websocket_message(NYN) :: {valid, valid_message(NYN)} | invalid.
...
-type handler_message(NYO) :: {internal, gramps@websocket:frame()} | {user, NYO}.

-type websocket_state(NYP) :: {websocket_state,
        bitstring(),
        NYP,
        gleam@option:option(gramps@websocket@compression:compression())}.

-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/websocket.gleam", 56).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/websocket.gleam", 264).
-spec get_messages(
    bitstring(),
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/websocket.gleam", 354).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/websocket.gleam", 361).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(NZX))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(NZX))).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/websocket.gleam", 277).
-spec apply_frames(
    list(gramps@websocket:frame()),
    fun((NZN, websocket_connection(), handler_message(NZO)) -> gleam@otp@actor:next(NZO, NZN)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(NZO), NZN),
    fun((NZN) -> nil)
) -> gleam@otp@actor:next(websocket_message(NZO), NZN).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/mist/src/mist/internal/websocket.gleam", 82).
-spec initialize_connection(
    fun((websocket_connection()) -> {NYY,
        gleam@option:option(gleam@erlang@process:selector(NYZ))}),
    fun((NYY) -> nil),
    fun((NYY, websocket_connection(), handler_message(NYZ)) -> gleam@otp@actor:next(NYZ, NYY)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(NYZ))} | {error, nil}.
