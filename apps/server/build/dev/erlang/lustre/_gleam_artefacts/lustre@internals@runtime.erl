...
-type state(UXF, UXG, UXH) :: {state,
        gleam@erlang@process:subject(action(UXG, UXH)),
        gleam@erlang@process:selector(action(UXG, UXH)),
        UXF,
        fun((UXF, UXG) -> {UXF, lustre@effect:effect(UXG)}),
        fun((UXF) -> lustre@internals@vdom:element(UXG)),
        lustre@internals@vdom:element(UXG),
        gleam@dict:dict(binary(), fun((lustre@internals@patch:patch(UXG)) -> nil)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, UXG} |
            {error, list(gleam@dynamic:decode_error())})),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, UXG} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type action(UXI, UXJ) :: {attrs, list({binary(), gleam@dynamic:dynamic_()})} |
    {batch, list(UXI), lustre@effect:effect(UXI)} |
    {debug, debug_action()} |
    {dispatch, UXI} |
    {emit, binary(), gleam@json:json()} |
    {event, binary(), gleam@dynamic:dynamic_()} |
    {set_selector, gleam@erlang@process:selector(UXI)} |
    shutdown |
    {subscribe, binary(), fun((lustre@internals@patch:patch(UXI)) -> nil)} |
    {unsubscribe, binary()} |
    {gleam_phantom, UXJ}.
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/lustre/src/lustre/internals/runtime.gleam", 233).
-spec run_renderers(
    gleam@dict:dict(any(), fun((lustre@internals@patch:patch(UYO)) -> nil)),
    lustre@internals@patch:patch(UYO)
) -> nil.
run_renderers(Renderers, Patch) ->
    gleam@dict:fold(Renderers, nil, fun(_, _, Renderer) -> Renderer(Patch) end).

-file("/home/raphaelac/repositories/lauraxv/server/build/packages/lustre/src/lustre/internals/runtime.gleam", 242).
-spec run_effects(
    lustre@effect:effect(UYT),
    gleam@erlang@process:subject(action(UYT, any()))
) -> nil.
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/lustre/src/lustre/internals/runtime.gleam", 94).
-spec loop(action(UXY, UXZ), state(UYC, UXY, UXZ)) -> gleam@otp@actor:next(action(UXY, UXZ), state(UYC, UXY, UXZ)).
loop(Message, State) ->
    case Message of
        {attrs, Attrs} ->
        ...
        {debug, {model, Reply}} ->
            Reply(gleam@dynamic:from(erlang:element(4, State))),
            gleam@otp@actor:continue(State);
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/lustre/src/lustre/internals/runtime.gleam", 60).
-spec start(
    {UXK, lustre@effect:effect(UXL)},
    fun((UXK, UXL) -> {UXK, lustre@effect:effect(UXL)}),
    fun((UXK) -> lustre@internals@vdom:element(UXL)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, UXL} |
        {error, list(gleam@dynamic:decode_error())}))
) -> {ok, gleam@erlang@process:subject(action(UXL, any()))} |
