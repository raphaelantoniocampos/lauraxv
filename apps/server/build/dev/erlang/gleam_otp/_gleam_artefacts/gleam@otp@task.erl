...
-opaque task(KFN) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(KFN))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(KFO) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, KFO}.

-file("/home/raphaelac/repositories/lauraxv/server/build/packages/gleam_otp/src/gleam/otp/task.gleam", 51).
-spec async(fun(() -> KFP)) -> task(KFP).
async(Work) ->
    Owner = erlang:self(),
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/gleam_otp/src/gleam/otp/task.gleam", 71).
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/gleam_otp/src/gleam/otp/task.gleam", 94).
-spec try_await(task(KFT), integer()) -> {ok, KFT} | {error, await_error()}.
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/gleam_otp/src/gleam/otp/task.gleam", 118).
-spec await(task(KFX), integer()) -> KFX.
await(Task, Timeout) ->
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/gleam_otp/src/gleam/otp/task.gleam", 129).
-spec try_await_forever(task(KFZ)) -> {ok, KFZ} | {error, await_error()}.
...
-file("/home/raphaelac/repositories/lauraxv/server/build/packages/gleam_otp/src/gleam/otp/task.gleam", 150).
-spec await_forever(task(KGD)) -> KGD.
await_forever(Task) ->
