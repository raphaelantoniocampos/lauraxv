...
-spec method_override(gleam@http@request:request(RYB)) -> gleam@http@request:request(RYB).
...
-spec fn_with_bad_request_error(fun((RZC) -> {ok, RZD} | {error, any()})) -> fun((RZC) -> {ok,
        RZD} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((RYW, bitstring()) -> {ok, RYW} |
        {error, gleam@http@response:response(body())}),
    RYW
) -> {ok, {gleam@option:option(buffered_reader()), integer(), RYW}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), RZU})) -> list({binary(), RZU}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, RZX} | {error, any()},
    fun((RZX) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, RYO} | {error, any()}) -> {ok, RYO} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
