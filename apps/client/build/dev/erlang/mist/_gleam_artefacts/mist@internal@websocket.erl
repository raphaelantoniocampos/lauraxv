...
-type valid_message(YCX) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, YCX}.

-type websocket_message(YCY) :: {valid, valid_message(YCY)} | invalid.
...
-type handler_message(YCZ) :: {internal, gramps@websocket:frame()} | {user, YCZ}.

-type websocket_state(YDA) :: {websocket_state,
        bitstring(),
        YDA,
        gleam@option:option(gramps@websocket@compression:compression())}.

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/mist/src/mist/internal/websocket.gleam", 56).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/mist/src/mist/internal/websocket.gleam", 264).
-spec get_messages(
    bitstring(),
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/mist/src/mist/internal/websocket.gleam", 354).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/mist/src/mist/internal/websocket.gleam", 361).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(YEI))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(YEI))).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/mist/src/mist/internal/websocket.gleam", 277).
-spec apply_frames(
    list(gramps@websocket:frame()),
    fun((YDY, websocket_connection(), handler_message(YDZ)) -> gleam@otp@actor:next(YDZ, YDY)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(YDZ), YDY),
    fun((YDY) -> nil)
) -> gleam@otp@actor:next(websocket_message(YDZ), YDY).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/mist/src/mist/internal/websocket.gleam", 82).
-spec initialize_connection(
    fun((websocket_connection()) -> {YDJ,
        gleam@option:option(gleam@erlang@process:selector(YDK))}),
    fun((YDJ) -> nil),
    fun((YDJ, websocket_connection(), handler_message(YDK)) -> gleam@otp@actor:next(YDK, YDJ)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(YDK))} | {error, nil}.
