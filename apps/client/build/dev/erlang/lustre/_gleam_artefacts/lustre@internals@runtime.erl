...
-type state(SXZ, SYA, SYB) :: {state,
        gleam@erlang@process:subject(action(SYA, SYB)),
        gleam@erlang@process:selector(action(SYA, SYB)),
        SXZ,
        fun((SXZ, SYA) -> {SXZ, lustre@effect:effect(SYA)}),
        fun((SXZ) -> lustre@internals@vdom:element(SYA)),
        lustre@internals@vdom:element(SYA),
        gleam@dict:dict(binary(), fun((lustre@internals@patch:patch(SYA)) -> nil)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, SYA} |
            {error, list(gleam@dynamic:decode_error())})),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, SYA} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type action(SYC, SYD) :: {attrs, list({binary(), gleam@dynamic:dynamic_()})} |
    {batch, list(SYC), lustre@effect:effect(SYC)} |
    {debug, debug_action()} |
    {dispatch, SYC} |
    {emit, binary(), gleam@json:json()} |
    {event, binary(), gleam@dynamic:dynamic_()} |
    {set_selector, gleam@erlang@process:selector(SYC)} |
    shutdown |
    {subscribe, binary(), fun((lustre@internals@patch:patch(SYC)) -> nil)} |
    {unsubscribe, binary()} |
    {gleam_phantom, SYD}.
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/lustre/src/lustre/internals/runtime.gleam", 233).
-spec run_renderers(
    gleam@dict:dict(any(), fun((lustre@internals@patch:patch(SZI)) -> nil)),
    lustre@internals@patch:patch(SZI)
) -> nil.
run_renderers(Renderers, Patch) ->
    gleam@dict:fold(Renderers, nil, fun(_, _, Renderer) -> Renderer(Patch) end).

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/lustre/src/lustre/internals/runtime.gleam", 242).
-spec run_effects(
    lustre@effect:effect(SZN),
    gleam@erlang@process:subject(action(SZN, any()))
) -> nil.
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/lustre/src/lustre/internals/runtime.gleam", 94).
-spec loop(action(SYS, SYT), state(SYW, SYS, SYT)) -> gleam@otp@actor:next(action(SYS, SYT), state(SYW, SYS, SYT)).
loop(Message, State) ->
    case Message of
        {attrs, Attrs} ->
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/lustre/src/lustre/internals/runtime.gleam", 60).
-spec start(
    {SYE, lustre@effect:effect(SYF)},
    fun((SYE, SYF) -> {SYE, lustre@effect:effect(SYF)}),
    fun((SYE) -> lustre@internals@vdom:element(SYF)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, SYF} |
        {error, list(gleam@dynamic:decode_error())}))
) -> {ok, gleam@erlang@process:subject(action(SYF, any()))} |
