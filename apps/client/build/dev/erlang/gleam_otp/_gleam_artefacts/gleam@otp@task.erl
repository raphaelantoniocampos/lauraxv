...
-opaque task(MMR) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(MMR))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(MMS) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, MMS}.

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/task.gleam", 51).
-spec async(fun(() -> MMT)) -> task(MMT).
async(Work) ->
    Owner = erlang:self(),
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/task.gleam", 71).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/task.gleam", 94).
-spec try_await(task(MMX), integer()) -> {ok, MMX} | {error, await_error()}.
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/task.gleam", 118).
-spec await(task(MNB), integer()) -> MNB.
await(Task, Timeout) ->
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/task.gleam", 129).
-spec try_await_forever(task(MND)) -> {ok, MND} | {error, await_error()}.
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/task.gleam", 150).
-spec await_forever(task(MNH)) -> MNH.
await_forever(Task) ->
