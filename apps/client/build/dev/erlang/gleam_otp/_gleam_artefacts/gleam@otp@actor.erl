...
-type message(LLF) :: {message, LLF} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(LLG, LLH) :: {continue,
        LLH,
        gleam@option:option(gleam@erlang@process:selector(LLG))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(LLI, LLJ) :: {ready, LLI, gleam@erlang@process:selector(LLJ)} |
    {failed, binary()}.

-type self(LLK, LLL) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        LLK,
        gleam@erlang@process:subject(LLL),
        gleam@erlang@process:selector(message(LLL)),
        gleam@otp@system:debug_state(),
        fun((LLL, LLK) -> next(LLL, LLK))}.

-type spec(LLM, LLN) :: {spec,
        fun(() -> init_result(LLM, LLN)),
        integer(),
        fun((LLN, LLM) -> next(LLN, LLM))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(LLO) :: {ack,
        {ok, gleam@erlang@process:subject(LLO)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 178).
-spec continue(LLV) -> next(any(), LLV).
continue(State) ->
    {continue, State, none}.

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 186).
-spec with_selector(next(LLZ, LMA), gleam@erlang@process:selector(LLZ)) -> next(LLZ, LMA).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 259).
-spec exit_process(gleam@erlang@process:exit_reason()) -> gleam@erlang@process:exit_reason().
exit_process(Reason) ->
    Reason.

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 295).
-spec selecting_system_messages(gleam@erlang@process:selector(message(LML))) -> gleam@erlang@process:selector(message(LML)).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 264).
-spec receive_message(self(any(), LMH)) -> message(LMH).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 308).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 415).
-spec init_selector(
    gleam@erlang@process:subject(LQZ),
    gleam@erlang@process:selector(LQZ)
) -> gleam@erlang@process:selector(message(LQZ)).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 318).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 374).
-spec initialise_actor(
    spec(any(), LNC),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(LNC)} |
        {error, gleam@erlang@process:exit_reason()})
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 444).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 466).
-spec start_spec(spec(any(), LNP)) -> {ok, gleam@erlang@process:subject(LNP)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 514).
-spec start(LNV, fun((LNW, LNV) -> next(LNW, LNV))) -> {ok,
        gleam@erlang@process:subject(LNW)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 529).
-spec send(gleam@erlang@process:subject(LOC), LOC) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/gleam_otp/src/gleam/otp/actor.gleam", 542).
-spec call(
    gleam@erlang@process:subject(LOE),
    fun((gleam@erlang@process:subject(LOG)) -> LOE),
    integer()
) -> LOG.
