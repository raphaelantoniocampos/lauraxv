...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/ranger/src/ranger.gleam", 68).
-spec create(
    fun((FTL) -> boolean()),
    fun((FTM) -> FTM),
    fun((FTL, FTM) -> FTL),
    fun((FTL, FTL) -> gleam@order:order())
) -> fun((FTL, FTL, FTM) -> {ok, gleam@iterator:iterator(FTL)} | {error, nil}).
...
-file("/home/raphaelac/repositories/lauraxv/client/build/packages/ranger/src/ranger.gleam", 144).
-spec create_infinite(
    fun((FTQ) -> boolean()),
    fun((FTQ, FTR) -> FTQ),
    fun((FTQ, FTQ) -> gleam@order:order())
) -> fun((FTQ, FTR) -> {ok, gleam@iterator:iterator(FTQ)} | {error, nil}).
