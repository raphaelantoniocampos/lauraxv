...
-type message(PIO) :: {internal, internal_message()} | {user, PIO}.

-type loop_message(PIP) :: {packet, bitstring()} | {custom, PIP}.

-type loop_state(PIQ, PIR) :: {loop_state,
        {ok, {glisten@transport:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(PIQ)),
        glisten@transport:transport(),
        PIR}.

-type connection(PIS) :: {connection,
        {ok, {glisten@transport:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(PIS))}.

-type handler(PIT, PIU) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(PIT), PIU, connection(PIT)) -> gleam@otp@actor:next(loop_message(PIT), PIU)),
        fun((connection(PIT)) -> {PIU,
            gleam@option:option(gleam@erlang@process:selector(PIT))}),
        gleam@option:option(fun((PIU) -> nil)),
        glisten@transport:transport()}.

-file("/home/raphaelac/repositories/lauraxv/client/build/packages/glisten/src/glisten/internal/handler.gleam", 73).
-spec start(handler(PJE, any())) -> {ok,
        gleam@erlang@process:subject(message(PJE))} |
